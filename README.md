...
## Overview
The purpose of this work was to develop a mapped time series of woody cover at MPG Ranch, a conservation property in western Montana, USA. The products will facilitate downstream ecological analyses, such as identification of factors driving bird abundance. One candidate for estimating woody cover is the Rangeland Analysis Platform (RAP; Allred et al. 2021), a machine learning model that predicts plant functional group cover, including shrubs and trees, across the continental US at a 30m resolution from 1986 to the present. While the spatial and temporal continuity of this product is robust, in practice I found that RAP predictions do not align with LiDAR ground truth at MPG Ranch. This repo contains the code and analyses for a process to calibrate RAP woody cover predictions with LiDAR ground truth at MPG Ranch. I prioritized the use of publicly available datasets and libraries for this task so that it may be replicated in other locations.

## Summary of key findings
The uncalibrated RAP model dramatically overestimates woody cover in upland grassland sites at MPG Ranch. These grassland estimates vary little from adjacent ecologically distinct zones. This presents challenges for ecological analyses seeking to contrast the relative influence of woody cover between, e.g., grasslands and shrubby draws or forested floodplains. The calibration process reduced woody cover overestimation in the grasslands, which represent the vast majority of the study area. The outcome of the calibration process was a strong overall benefit to model performance, enabling it to extrapolate well to areas and time periods outside of its training data. I observed a 1.26-fold increase in rank order agreement, a 2.8-fold increase in R² score, and a 2.22-fold decrease in mean absolute error relative to uncalibrated RAP predictions in these test areas. Yet, spatially clustered pockets of woody cover error remained in the floodplain, mid-elevation regions with dense shrubs, and higher elevations of mixed conifer systems. It is important to note that, while absolute errors remain in some areas, rank order agreement of predictions and ground truth across the landscape was high. This suggests that detecting correlations among woody cover and other ecological measures may yet prove tractable as areas of relatively high and low cover map as expected to high and low cover areas in LiDAR-derived ground truth.
